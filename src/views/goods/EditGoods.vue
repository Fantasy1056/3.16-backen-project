<template>
  <div class="add-goods">
    <BreadCrumb frist="商品管理" second="商品编辑"></BreadCrumb>
...
const getGoodsById = async () => {
  const { data: res } = await reqGetGoodsById(parseInt(route.params.id))

  reqInfo.goods_name = res.data.goods_name
  reqInfo.goods_cat = res.data.goods_cat
  reqInfo.goods_price = res.data.goods_price
  reqInfo.goods_number = res.data.goods_number
  reqInfo.goods_weight = res.data.goods_weight
  reqInfo.goods_introduce = res.data.goods_introduce
  reqInfo.pics = res.data.pics

  selectedKeys.value.push(res.data.cat_one_id as number)
  selectedKeys.value.push(res.data.cat_two_id as number)
  selectedKeys.value.push(res.data.cat_three_id as number)

  reqInfo.pics.forEach((item) => {
    fileList.value.push({ name: item.pics_id, url: item.pics_big_url })
  })
}
...
const addGoods = () => {
  addGoodsForm.value.validate(async (valid: any) => {
    ...
    const { data: res } = await reqEditGoodsById(
      parseInt(route.params.id),
      reqInfo
    )

    if (res.meta.status !== 200) return ElMessage.error('编辑商品失败!')
